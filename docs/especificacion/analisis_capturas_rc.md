...
**Primer hallazgo, antes de mirar el contenido**: no son screenshots — son **fotos de la pantalla del RC tomadas con un celular** y enviadas por WhatsApp (reflejos, cortes, ángulo, texto parcial). Eso ya define dos cosas: (a) la app en el RC debe capturar **screenshot nativo** (corre en el mismo dispositivo), y (b) ningún dato crítico puede depender de OCR sobre estas fotos — la captura es evidencia, la transcripción es manual y mínima.
...
- **Transcripción mínima guiada** al cerrar: los 4 números del desglose (trabajo/pendiente/margen/obstáculo) + tiempo + litros + L/ha — con la validación aritmética de §3 en el formulario, offline.
