...
- `personas` — id, nombre (completo, lo compone el servidor), nombres, apellido_paterno, apellido_materno, ci (única entre personas vivas), celular, correo, direccion, rol, tarifa_ha (piloto/auxiliar), sueldo_mensual (jefe/encargado), base_id, activo. *Nota del 21/9/2026 (pedido del dueño): datos personales y de referencia separados — a quien se le confía un dron hay que poder identificarlo y ubicarlo. `rol` es el puesto de campo, no el acceso al sistema (eso es `sec_user`). `tarifa_ha` sigue acá hasta que se decida el esquema de pago por cuadrilla (por hectárea o por día); ver pendientes.*
