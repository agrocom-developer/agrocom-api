...
Migrado en `seguridad::pages.usuarios.index` y `seguridad::pages.bitacora.index` (piloto, 15/9/2026); `comercial::pages.reportes-comerciales.index` migró en la tarea 120 (20/9/2026); las pantallas que todavía usan `alert-strip` para este caso (`personal::pages.personas.desempeno`, y cualquier otra del rollout de filtros) migran al mismo patrón cuando les toque su pasada.
