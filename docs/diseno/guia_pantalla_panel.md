...
2. **Dos columnas de campos dentro de la tarjeta, no una.** `grid-template-columns: repeat(auto-fit, minmax(14rem, 1fr))`; un campo que necesita el ancho completo (dirección, logo, textarea) declara `grid-column: 1 / -1`. Un formulario de una sola columna angosta desperdicia toda la mitad derecha de la pantalla — es lo que hace hoy `organizacion` con su `max-width: 600px`.
