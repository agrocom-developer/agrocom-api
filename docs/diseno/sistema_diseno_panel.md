...
| `--ag-font-family-display` | Tipografía de titulares grandes tipo hero (HU-02, refinamiento editorial). Aditivo, no reemplaza `-base` en ningún otro lado — hoy solo la usa `templates/auth-layout` (wordmark + headline). "Instrument Serif" declarada con fallback serif del sistema; **pendiente de instalación** (`@fontsource/instrument-serif` no está en `package.json` a la fecha) → `frontend`, mismo criterio ya usado con Material Symbols antes de que se instalara (ver §6) |
...
- **`--ag-font-family-display`** (nuevo, `tokens/primitives/base.css`): tipografía de titulares grandes ("Instrument Serif"), ADITIVA a `--ag-font-family-base` (IBM Plex Sans sigue siendo la única familia de texto/formularios/menú). **No instalada todavía** (`@fontsource/instrument-serif` no está en `package.json`) — el token queda con fallback serif del sistema, mismo criterio que se usó con Material Symbols antes de que `frontend` lo instalara (ver §6). → `frontend`, cuando lo priorice.
...
- **El wordmark es texto, no el átomo `logo`**: `logo-light.jpeg`/`logo-dark.jpeg` traen fondo sólido horneado (JPEG, sin transparencia — limitación ya documentada en `logo.css`); sobre una foto se vería un recuadro blanco/negro. Se resolvió como `<p>` en `--ag-font-family-display` reutilizando la clave `ui.logo.alt` (sin agregar un string nuevo) — el logo de `login-form` no se duplicó ni se quitó, sigue siendo el único wordmark visible en mobile (donde la foto se oculta).
...
- **Asset de logo transparente** (hoy `logo-light.jpeg`/`logo-dark.jpeg` traen fondo sólido horneado en el archivo, no transparencia) — solo es un problema si el logo llega a usarse fuera del chrome de marca, donde su fondo coincide con `--ag-color-bg-chrome`. No bloqueante para HU-02 (por eso el wordmark editorial de `auth-layout` se resolvió como texto, no como esta imagen — ver notas de §3).
- **Instalar `@fontsource/instrument-serif` vía npm** (refinamiento editorial de `auth-layout`, segunda vuelta de HU-02): `--ag-font-family-display` está declarado en `tokens/primitives/base.css` con fallback serif del sistema, funciona ya (se ve una serif del SO en vez de "Instrument Serif"), pero no está auto-hospedada. Mismo criterio que Material Symbols antes de instalarse. → `frontend`, cuando lo priorice; no bloqueante.
