...
| Molecule | `step-arrow` | `resources/views/components/molecules/step-arrow.blade.php` | Implementado (19/9/2026 — primer consumidor `campanias/edit`, vía `_formulario` + `_cambio-estado`). La máquina de estados de un objeto como pasos con forma de flecha, COMPARTIDO: los pasos los arma `Compartido\Infraestructura\Http\PasosDeEstado::armar()` leyendo la tabla de transiciones de la propia máquina (`completed`/`current`/`next`/`pending`/`blocked`), y `PasosDeEstado::ayuda()` compone el párrafo (`help`) con el texto que el objeto define en su `lang` por estado (`<modulo>.<objeto>.estado_ayuda.<valor>`, el porqué de avanzar) más el cierre genérico de `ui.pasos.*`. Un paso `next` es un botón que abre el `confirm-modal` de la página; el componente no envía ni decide nada. Tema: el paso actual lleva relleno sólido del color del estado con texto `--ag-color-gray-0` (constante, como `atoms/badge`; `--ag-color-text-inverse` se invierte en oscuro y NO sirve sobre un `-contrast-fill`), salvo el tono `neutral`, que va solo con contorno de 2px, y salvo cuando es el ÚLTIMO paso (ya no hay a dónde avanzar): ahí se dibuja como procesado (color suave + check, igual que un paso completado) y solo conserva `aria-current`. Tamaño: ocupa todo el ancho de su fila (col-12) y consulta el ancho de su propio contenedor (`container: ag-step-arrow`), no el del viewport — compacto bajo 48rem y apilado sin puntas bajo 32rem; pensado para 3 a 4 pasos. |
