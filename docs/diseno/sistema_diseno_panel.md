...
| Texto blanco sobre `--ag-color-warning-contrast-fill` (amber-700, `#e8720c`) | ~3.03:1 | **Falla AA** | Por eso warning NO usa texto blanco — ver fila siguiente |
| Texto gris-900 sobre `--ag-color-warning-contrast-fill` (amber-700, `#e8720c`) | ~5.09:1 | Pasa AA (texto normal, 4.5:1) | `.ag-badge--warning` — es TEXTO real ("Sin evidencia"), exige 4.5:1; blanco falla ahí (fila de arriba) |
...
| Atom | `time-range` | `resources/views/components/atoms/time-range.blade.php` | Implementado (19/9/2026 — primer uso: horario por lote en `contratos/_lotes-tabla`, reemplaza a los dos `<input type="time">` nativos). Rango horario en UNA casilla ("06:00 – 10:00") con un selector de reloj circular estilo Material: `<dialog>` modal con cabecera de color primario (la hora grande, cuatro segmentos pulsables: hora y minutos de inicio y de fin), dial de 24 h (anillo exterior 12·1–11, interior 00·13–23; minutos con marcas cada 5 y precisión de 1 al arrastrar), y pie con teclado / Limpiar / Cancelar / Aceptar. Elegir una hora pasa sola a sus minutos, luego a la hora de fin y a sus minutos. Modo de texto "HH:MM" como alternativa accesible; valida que ambas horas estén y que el fin sea posterior. Siempre 24 h (`H:i`, el que espera `date_format:H:i`), nunca a. m./p. m. Props `nameStart`, `nameEnd`, `id` (único: de él salen los del diálogo), `valueStart`, `valueEnd`, `label`, `help`, `error`, `invalid`, `disabled`. Mejora progresiva de `atoms/date`: los dos nativos son los controles reales y se ocultan con `--enhanced`; al confirmar escribe `.value` y dispara `input` y `change`. Cualquier código puede deshabilitarlo cambiando `disabled` en los nativos (el componente lo observa). Las filas que nacen después de cargar la página se inicializan con `initTimeRanges(raiz)` (idempotente) y se clonan de un `<template>` con `__INDICE__` en `name` e `id`. Tokens: cabecera `--ag-color-primary` con texto `--ag-color-gray-0` (constante, como el paso actual de `step-arrow`), dial `--ag-color-bg-input-chrome`. Verificado en claro y oscuro y a 360 px. |
