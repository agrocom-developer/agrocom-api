...
**Adenda del 19/9/2026 (ubicación del formulario de Propiedades, pedido directo):** el select de **Municipio** deja de depender de la provincia. Lista todos los municipios del catálogo con la etiqueta "Municipio - Departamento" (hay nombres repetidos entre departamentos) y es un select con búsqueda por palabras; al elegir uno se completan solos el departamento y la provincia que le corresponden, pisando lo que hubiera — para el caso de conocer solo el municipio, o de haber elegido mal el departamento. Departamento sigue filtrando las provincias, y cambiar departamento o provincia a uno que no contiene al municipio lo limpia. Es puro comportamiento de pantalla (`propiedades-form.js`): el modelo y `ValidadorUbicacionGeografica` no cambian. Además, la propiedad **siempre tiene color**: el formulario lo preselecciona en verde (`ColorPropiedad::porDefecto()`) y `CrearPropiedad`/`ActualizarPropiedad` lo completan si llega vacío.
