...
**Guarda de imputación:** ninguna escritura nueva puede imputarse a una campaña `cerrada`. Lo verifica el módulo dueño de cada tabla al crear, no un trigger. *Adenda 19/9/2026 (pedido directo del dueño): la guarda pasa a exigir campaña `abierta` — una `planificada` todavía no admite contratos, gastos ni combustible hasta que se abre (`Campania\Contratos\DatosCampania::admiteImputaciones()` es quien lo decide; `Comercial` y `Finanzas` solo reaccionan con `CampaniaNoAbierta`). Los selects de los formularios ofrecen solo campañas abiertas. Un contrato que ya tenía una campaña `planificada` se sigue pudiendo editar sin cambiarla; uno en campaña `cerrada` sigue sin admitir cambios.*
...
## Adenda del 19/9/2026 — «cuadrilla», su equipamiento, y la estadía también desde el panel

Pedido directo del dueño, sobre el punto 3 (equipo de trabajo) y sobre la estadía en hacienda (HU-51).

**1. En pantalla se llama «cuadrilla»; en el código sigue siendo `equipo_trabajo`.** Una sesión anterior había rotulado «Escuadras» un ítem del menú; en el agro nadie dice «escuadra» (es el instrumento de medir o la unidad militar) y sí «cuadrilla» —«cuadrilla de cosecha», «cuadrilla de fumigación»—, que es además como este mismo ADR ya la describía. De paso resuelve la ambigüedad de «equipo», que en el sistema también nombra el equipamiento y los casilleros «Equipo 1 / Equipo 2» de una orden de trabajo. Se renombra todo lo VISIBLE (menú, textos, URL `/panel/cuadrillas`, nombres de ruta `panel.cuadrillas.*`); tablas, modelos, permisos y el `equipo_trabajo_id` del contrato de sync con `agrocom-field` no cambian — renombrarlos toca el contrato con la app de campo sin ganar nada para el usuario. El glosario deja escrita la equivalencia.

**2. El ítem de menú vive en Operación.** «Cuadrillas» ocupa el lugar que tenía el reparto de hectáreas por orden (tarea 85), que sale del menú porque el alta de Orden de Trabajo ya lo cubre; esa pantalla queda como ficha de cada orden (`panel.reparto-cuadrillas.*`), a la que se llega desde la orden de aplicación. El ítem «Equipos de trabajo» de Recursos se MUEVE (misma fila de `sec_menu`), no se duplica. El backend sigue en `Personal`: agrupar bajo Operación es de layout (ADR 0011, extensión del 26/8/2026, punto 3).

**3. La cuadrilla se arma de una sola vez y lleva todo lo que sale al campo.** El alta pide piloto, ayudante (con un segundo opcional) y dron, y admite camioneta, generador y baterías; integrantes y equipamiento siguen teniendo vigencia propia (punto 3 original), solo que ahora nacen junto con la cuadrilla en una misma transacción (`ArmarCuadrilla`). Se suman dos cosas al equipamiento:
- **Baterías** como cuarto `recurso_tipo`. Cada batería es una unidad identificada de `man_baterias`; la «cantidad de baterías» es cuántas tiene vigentes la cuadrilla, no un número que se tipea y que nadie puede contrastar.
- **Accesorios** (machete, palas, linternas): catálogo propio `per_accesorios` + `per_equipo_accesorios` con cantidad. Vive en `Personal` y no en `Inventario` porque hoy no tiene stock ni costo: es la lista de lo que lleva la cuadrilla. Si algún día se quiere controlar existencias de herramientas por base, el catálogo migra a `Inventario` y `Personal` lo lee por contrato.

Los catálogos de otros módulos (drones, vehículos, generadores, baterías) se leen por contrato —`Operaciones\Contratos\LecturaDrones`, `Mantenimiento\Contratos\LecturaEquipamiento`—; el `DB::table` ajeno que tenía el controlador se retira.

**4. `estado` de la cuadrilla pasa por una tabla de transiciones.** Era un `<select>` del formulario (activo/inactivo), que es justo lo que la invariante 7 prohíbe. Ahora es `activa ⇄ inactiva` en `TransicionesEquipoTrabajo`, se cambia con los pasos de la ficha (`CambiarEstadoEquipoTrabajo`) y una cuadrilla nueva nace activa.

**5. La estadía también la registra la oficina.** HU-51 la había dejado de solo lectura en el panel («nace en la app de campo»). La app todavía no está distribuida y el dato hace falta igual para justificar el gasto imputado a la cuadrilla, así que el panel gana alta, edición (solo mientras está en curso), finalización y baja, con `uuid_cliente` generado en el servidor — mismo criterio que los trabajos que nacen del panel. El sync no cambia de forma: solo acepta un campo opcional más. El estado sigue sin columna: se deriva de `salida` (`en_curso → finalizada`, sin vuelta atrás) y la tabla de transiciones existe para que la finalización pase por una guarda y la ficha pueda dibujar sus pasos.

**6. `tipo_alojamiento`: dónde durmió la cuadrilla.** Tres valores —en la hacienda, en el pueblo más cercano, camping en la propiedad—. Sale de las respuestas de campo (el jefe de campo: «en algunas propiedades no nos proporcionan comida», viáticos de «alojamiento, comida, agua») y es lo que explica por qué dos estadías de los mismos días cuestan distinto. Obligatorio desde el panel, opcional por sync.

**7. Base y estadía no son lo mismo, y ninguna sale del menú.** Se evaluó (19/9/2026) si «Bases» de Recursos y «Estadías» de Operación eran la misma cosa, porque en el relato del negocio la «base» es el campamento que se monta en la propiedad. En el modelo son preguntas distintas: la **base** (`per_bases`) es un catálogo fijo de Agrocom al que pertenecen personas, cuadrillas, vehículos, baterías, generadores y el stock de repuestos, y al que se imputan gastos (diez tablas la referencian: personas y cuadrillas; vehículos, generadores y baterías; stock y movimientos de inventario; gastos, combustible y rendiciones); la **estadía** es un hecho con fechas — qué cuadrilla estuvo en qué propiedad de qué cliente, desde cuándo y hasta cuándo. Quitar cualquiera de las dos deja sin respuesta una de las preguntas. El «campamento» del relato queda cubierto por el `tipo_alojamiento` de la estadía.
