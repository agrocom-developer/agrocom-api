...
- En la práctica, HU-32 (avance comercial por contrato) y HU-41 (portal: actas y reportes técnicos por contrato) resolvieron su agregación con contratos de lectura cross-módulo (`Contratos/Lectura*`) que hidratan agregados Eloquent en PHP — exactamente la alternativa descartada más arriba. Para volumen bajo (un contrato, una campaña) el costo de esa hidratación es despreciable y evita la fricción de versionar una vista por cada lectura nueva; queda aceptado como patrón para reportes acotados por contrato/campaña. `vw_*`/vistas materializadas siguen siendo la vía obligatoria para lecturas realmente pesadas — dashboards y tableros que agregan sobre toda la base, no sobre el recorte de un solo contrato.
