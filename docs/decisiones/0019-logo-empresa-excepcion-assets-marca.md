...
- **`mimes:png,svg`**, no la regla `image` de Laravel: `image` excluye SVG explícitamente (por el mismo riesgo de XSS que se señala abajo), y la ayuda visual ya vigente en pantalla (`campo_logo_ayuda`: *"PNG o SVG, fondo transparente recomendado"*) ya le prometió SVG al usuario — restringir a menos de lo que la pantalla anuncia sería un bug de UX, no una decisión de arquitectura nueva.
- **`max:2048`** (2048 KB = 2 MB): el ejemplo mock actual (`mock_logo_peso`, "240 KB") confirma que un logo real pesa un orden de magnitud menos; 2 MB es margen generoso sin dejar de acotar el disco.
...
- **Aceptar cualquier tipo de imagen (jpg, webp, etc.).** La ayuda visual ya vigente en pantalla fija el contrato con el usuario en PNG/SVG; ampliar el tipo de archivo aceptado sin que nadie lo haya pedido es una decisión de producto que no corresponde tomar en este ADR.
