...
## Adenda (19/9/2026) — El bloqueo del punto 9 se explica en el panel, no solo se rechaza

**Contexto.** El punto 9 exige cerrar o cancelar la aplicación abierta antes de cancelar o finalizar el contrato, y hasta ahora lo hacía solo con un error después de confirmar. Al llevar el cambio de estado del contrato a los pasos de su ficha de edición (`molecules/step-arrow`), el dueño pidió que ese bloqueo se muestre como un aviso que explique por qué no se puede todavía y a dónde ir — la restricción es informativa para quien la lee: la causa y el motivo de un cierre anticipado se indican al cancelar la orden (punto 5) — y que la regla «primero la orden, después el contrato» valga igual en los pasos que en el listado.

**Decisión.** La regla no cambia: sigue firme en el servidor (`MaquinaEstadosContrato`, `ContratoConAplicacionAbierta`), también ante un pedido directo que se salte la pantalla. Lo que cambia es lo que ve el usuario: con una aplicación abierta, «Ejecutado» y «Cancelado» —en los pasos de la ficha y en las acciones del listado— abren un modal informativo (`molecules/info-modal`) en lugar de la confirmación. Dice qué aplicación sigue abierta y en qué estado, pide cerrarla o cancelarla primero y ofrece ir a la orden. No lleva botón de confirmar: no se ofrece nada que el servidor vaya a rechazar. El modal usa el color del estado al que se quería pasar, igual que la confirmación que habría abierto.
