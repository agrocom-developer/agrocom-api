...
**Sprint 1 en curso (27/8/2026).** TE-01 parcial: el esqueleto Laravel de `agrocom-api` existe y la suite (Pint + Larastan + Pest) corre en CI; de TE-01 faltan el repo `agrocom-field` (Flutter) y el entorno staging (diferido — ADR 0010, el servidor no está definido). TE-03 (migraciones del núcleo comercial: clientes, contratos, campos, lotes, órdenes) **hecho** — PR #9, commit `02c6927`. TE-02 (spike de hardware en el RC real) sigue pendiente. HU-01 (usuarios multi-rol) está en curso en `feature/usuarios-multirol`, sin código todavía — ver punto 4 de "Próximo paso inmediato".
