...
| TE-17 | i18n real (español + al menos otro idioma): `flutter_localizations` + `intl`, extracción de todos los strings hardcodeados, selector de idioma persistido en `TE-16`. **Excepción explícita a "dominio en español" de `CLAUDE.md`** (decisión del 10/9/2026) — ADR propio de `agrocom-field` que fija el alcance exacto (la UI se traduce; el vocabulario de dominio interno — nombres de tabla, campos, `uuid_cliente`, estados — sigue en español fijo, igual que hoy) | Ningún string de UI hardcodeado en un widget; build pasa con 2 locales configurados; el ADR deja explícito qué NO se traduce | 2,0 d — **crítica** (toca una convención transversal de ambos repos) |
...
| HU-60 | Como piloto, quiero ver el pronóstico del clima del lote al abrir condiciones, además de poder ingresarlo a mano si no hay señal, para decidir con mejor información | API de clima consultada solo con conectividad; sin señal, el flujo manual de HU-06 sigue igual — el clima por API nunca bloquea el flujo offline | 1,5 d |
| HU-61 | Como piloto o auxiliar, quiero ver el lote en un mapa con mi ubicación, para orientarme en el campo | Mapa con el polígono del lote (mismo GeoJSON de HU-24/HU-56) cacheado localmente tras el último pull de catálogo; funciona sin señal con el último dato bajado | 2,0 d |
| HU-62 | Como piloto o auxiliar, quiero recibir avisos locales del propio dispositivo (batería caliente, sesión abierta hace muchas horas, orden nueva recién sincronizada), sin depender de señal para dispararse | Notificaciones locales generadas por reglas que corren en el dispositivo — sin servidor push, sin FCM (coherente con la decisión de HU-20) | 1,0 d |
| HU-63 | Como piloto o auxiliar, quiero ver en la app las alertas que me involucran (p. ej. mi sesión fue rechazada, ver HU-14), para enterarme sin esperar al panel | Vista de solo lectura sobre lo que ya bajó el pull de catálogo/sync — no es una fuente de datos nueva, consume lo que HU-19 ya genera del lado servidor | 1,0 d |
| HU-64 | Como piloto o auxiliar, quiero registrar una pausa con su causa (clima, mecánica, espera de caldo, etc.) desde la app, sin señal, para que quede constancia del tiempo perdido en el momento en que ocurre | Pausa con `uuid_cliente` vía outbox (mismo patrón que incidencias); llega al panel para HU-44 sin que el jefe de campo tenga que cargarla después | 1,5 d |
| HU-65 | Como piloto o auxiliar, quiero registrar un reclamo/queja durante el trabajo, para dejar constancia | **Bloqueada — alcance sin definir**: ¿queja sobre el servicio/producto que entrega el cliente (caldo, acceso al lote), sobre condiciones de trabajo del propio operario, o ambas? Sin esa definición no hay criterio de aceptación que escribir. Se resuelve con el usuario antes de entrar a la cola del ciclo (TE-18 no la toma bloqueada) | — |
| HU-66 | Como piloto o auxiliar, quiero ver mis devengos del período y el detalle de lo que apliqué (hectáreas, sesiones) desde la app, para saber qué voy a cobrar sin esperar señal | Datos del último pull sincronizado (mismo backend de HU-16/HU-28); vista de solo lectura — nunca calcula devengo en el dispositivo, la autoridad sigue siendo el servidor | 1,5 d |
| HU-67 | Como piloto o auxiliar, quiero consultar qué repuestos y suministros tiene mi base/equipo disponibles, para saber si puedo resolver algo en el momento | Vista de solo lectura sobre el catálogo de HU-27/HU-36 (drones, stock), bajado en el pull de catálogo | 1,0 d |
...
**Total: 19,5 d (sin contar HU-65, bloqueada) · Entrega: `agrocom-field` con desarrollo automatizado propio y las funciones esenciales de campo más allá del esqueleto de sincronización**

**Interacción piloto↔auxiliar (decisión del 10/9/2026): no se agrega HU nueva.** Cada rol ya ve el trabajo del otro a través del pull de catálogo/sync normal (HU-04/HU-05/HU-13) — sin mensajería ni comunicación directa entre dispositivos, coherente con la invariante 4 de `CLAUDE.md` de `agrocom-field` ("un solo rol escritor por tipo de registro") y con no tener conectividad garantizada en el lote.

**Evidencias — Google Drive descartado como canal, a favor del pipeline propio.** El pedido de "subir a la cuenta Drive" se resuelve con el pipeline ya planificado (TE-07: cola de evidencias comprimidas <300 KB, hash SHA-256, subida en segundo plano con reintentos) en vez de agregar Drive como canal alternativo — dos backends de evidencia duplicarían la lógica de reintento/estado sin necesidad real. Revisarlo si en el futuro aparece un motivo concreto (cuota de almacenamiento propio, por ejemplo).

**Orden real de ejecución**: TE-18 primero (el ciclo necesita existir antes de que le sigan encolando tareas), después TE-15/TE-16 (infraestructura que el resto consume), TE-17/HU-59 juntas (tema e idioma), y el resto en cualquier orden — son independientes entre sí. HU-65 queda fuera de la cola hasta que se defina su alcance.
