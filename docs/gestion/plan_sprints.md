...
| HU-25 | Como **encargado**, quiero crear y seguir las órdenes de aplicación desde el panel, para que el piloto las reciba en el pull de catálogo | ABM sobre `ope_ordenes_aplicacion` con su máquina de estados; una orden nueva aparece en `GET /api/sync/catalogo`. *Nota del 19/9/2026: la máquina de estados de la orden se amplió (`pausada`, `cancelada`), el número de aplicación pasó a ser correlativo y solo hay una orden abierta por contrato — ver HU-97 (Sprint 20) y ADR 0022.* | 1,5 d |
...
| HU-77 | Como **encargado**, quiero elegir si la campaña es de invierno o verano y que el nombre se arme solo (`Estación/AñoInicio/AñoFin`), para no tipear un nombre cada vez | `cpn_campanias` gana `estacion` (`invierno/verano`); `nombre` se autogenera si no se especifica; el panel puede *mostrar* "Activa"/"Inactiva" como etiqueta — "Activa" solo mientras la campaña está `abierta` (corregido el 19/9/2026: antes también lo era `planificada`); `planificada` y `cerrada` son "Inactiva" —, pero la máquina sigue siendo irreversible desde `cerrada` (ADR 0015) — sin excepción nueva | 1,0 d |
...
**Tramo siguiente: HU-97 (Sprint 20).** El cierre automático del contrato al
cerrarse su última aplicación, y la reforma de la Orden de Aplicación que el
dueño dejó decidida el mismo día —la orden pasa a ser siempre por toda la
aplicación del contrato, sin elegir lotes ni hectáreas parciales por lote— son
la HU-97 del Sprint 20 (ADR 0022). Esa reforma supera parcialmente a HU-92 (lo
de "orden con N lotes y hectáreas solicitadas por lote") y no cambia el reparto
por equipo y lote de la Orden de Trabajo (HU-70). Está registrada en
`docs/negocio/observaciones_operaciones_comercial_2026-09-18.md`.

---

## Sprint 20 — Órdenes de aplicación: una aplicación completa por orden (ronda del dueño, 18/9/2026)

*Objetivo: que cada orden de aplicación sea una aplicación completa del contrato,
con número correlativo y de a una por vez, y que el contrato se ejecute
aplicación por aplicación hasta finalizar solo al cerrarse la última. Corrige el
enfoque de HU-92 (orden con selección de lotes y hectáreas parciales por lote) y
completa lo que el Sprint 19 dejó manual (`finalizar`). Detalle de la corrección
del dueño en `docs/negocio/observaciones_operaciones_comercial_2026-09-18.md`
(sección 3); el porqué de la decisión, en el ADR 0022, que se apoya en el
ADR 0021. La numeración sigue desde HU-97. Implementada en la rama
`feature/orden-correlativa`, pendiente de merge.*

| ID | Historia / tarea | CA esenciales | Est. |
|---|---|---|---|
| HU-97 | Como **encargado**, quiero que cada orden de aplicación sea una aplicación completa del contrato, con número correlativo y una sola abierta por vez, y poder pausarla, cerrarla o cancelarla desde el panel, para que el contrato se ejecute aplicación por aplicación y finalice solo al cerrarse la última | **1) Aplicación completa:** una orden es UNA aplicación de todas las hectáreas y lotes del contrato (sus hectáreas son las `hectareas_contratadas`); no se agregan ni se quitan lotes a una orden ni hay hectáreas parciales por lote. `ope_orden_lotes` se conserva como copia automática, tomada al emitir la orden, de TODOS los lotes del contrato con `hectareas_solicitadas` = hectáreas completas del lote; `GET /api/sync/catalogo` mantiene su forma. **2) Número correlativo:** lo calcula el servidor (1, 2, 3…) y ya no se elige; un contrato sin órdenes parte de la aplicación 1; no se puede pasar de `aplicaciones_previstas`. **3) Cuándo se emite:** solo si el contrato está `vigente` ("En Ejecución"; se revoca el criterio anterior que permitía cualquier estado), no tiene una aplicación abierta (`emitida`, `vigente` o `pausada`) y no agotó sus aplicaciones. Garantía en base de datos: índice único parcial "una orden abierta por contrato" (`ope_ordenes_aplicacion (contrato_id) WHERE deleted_at IS NULL AND estado IN ('emitida','vigente','pausada')`) e índice único parcial `(contrato_id, nro_aplicacion)` que excluye las canceladas por fuerza mayor. **4) Máquina (invariante 7, servicio `MaquinaEstadosOrden`):** `emitida → vigente`; `vigente → pausada \| consumida \| cancelada`; `pausada → vigente \| cancelada`; `consumida`, `cancelada` y `vencida` sin salida; `vencida` sigue sin disparador de negocio; solo las `emitida` se editan y eliminan (baja lógica). **5) Pausar, cerrar, cancelar:** pausar exige motivo escrito y reanudar no pide nada; cerrar (`vigente → consumida`) es una acción manual del encargado con el informe del equipo a la vista, nada cierra la orden solo; cancelar (`vigente \| pausada → cancelada`) exige causa (`cliente` \| `fuerza_mayor`) y motivo escrito, y la falta de pago se registra como causa `cliente` más su motivo. **6) Numeración y causa:** una aplicación cancelada por fuerza mayor NO consume su número (se rehace con el mismo); una cancelada por causa del cliente SÍ lo consume (la siguiente lleva el que sigue). **7) Solo el panel decide:** la app de campo nunca pausa, cierra ni cancela una orden; lo que registra son incidencias (`ope_incidencias`) y pausas de sesión (`ope_pausas`) en los trabajos, y la orden muestra un badge informativo "Con inconvenientes" (con conteo) cuando sus trabajos las tienen — sin cambiar estados solo; el detalle vive en las órdenes de trabajo. **8) Cierre del contrato:** al cerrarse la última aplicación (nro de la orden cerrada ≥ `aplicaciones_previstas`) el contrato pasa solo a `finalizado` ("Ejecutado") y libera sus lotes (evento de dominio `AplicacionCerrada`, oyente `FinalizarContratoPorUltimaAplicacion` en Comercial; solo si el contrato está `vigente`); una aplicación cancelada no lo dispara. **9) Guarda del contrato:** un contrato no se cancela ni se finaliza mientras tenga una aplicación abierta (primero se cierra o se cancela esa aplicación); con la aplicación cancelada sí, aunque queden aplicaciones pendientes; la guarda vive en `MaquinaEstadosContrato` y lee las órdenes por `LecturaResumenOrdenesContrato` (campo `abiertas`). **10) Sin choque de lotes en órdenes:** la guarda "orden vigente duplicada en lote" se elimina; los choques se garantizan antes, entre contratos (ADR 0021). **11) Datos y permisos:** columnas nuevas en `ope_ordenes_aplicacion` (`motivo_pausa`, `pausada_at`, `reanudada_at`, `cerrada_at`, `cancelada_at`, `causa_cancelacion`, `motivo_cancelacion`); `CHECK` de estado con `pausada` y `cancelada` (solo Postgres); permisos `operaciones.orden.pausar` (pausar y reanudar), `operaciones.orden.cerrar` y `operaciones.orden.cancelar`; las migraciones fallan a propósito, con un mensaje que nombra los contratos, si la base ya trae órdenes con número repetido o varias abiertas por contrato. **12) UI:** el índice de órdenes muestra el número de aplicación ("2 de 3"), la cantidad de lotes del contrato, la cantidad de órdenes de trabajo realizadas de esa orden y el badge de inconvenientes; la sección "Lotes" de la orden (formulario y detalle) es una lista de solo lectura con el código de cada lote del contrato y su propiedad; en el contrato, el botón "Nueva orden de aplicación" se deshabilita si el contrato no está vigente, tiene una aplicación abierta o ya completó sus aplicaciones. **13) Pendientes conocidos (no resueltos en esta HU):** (a) la app de campo no se entera de que una orden fue cancelada, pausada o cerrada, porque el catálogo solo entrega órdenes `vigente` y el sync no tiene forma de retirar registros; (b) las sesiones ya abiertas en el campo no se frenan al pausar o cancelar; (c) si se agrega un lote al contrato mientras hay una aplicación abierta, esa aplicación no lo incluye (la siguiente sí) | 4,0 d |

**Total: 4,0 d · 0 pantallas nuevas de menú (amplía Órdenes de aplicación y Contratos)**

**Es crítica**: toca el servicio de la máquina de estados de la orden y el del
contrato (nueva guarda de cancelar y finalizar) — se implementa y se integra
igual; la revisión línea por línea es posterior, anotada en
`runs/revision-pendiente.txt` (regla de `CLAUDE.md` y
`automatizacion_desarrollo.md` §5).

**Relación con HUs existentes.** HU-25 (ABM de órdenes) y HU-92 (orden con N
lotes) se **corrigen**: la máquina de estados de la orden se amplía y el
enfoque de "elegir lotes y hectáreas por orden" queda superado; ver la nota en
cada una. HU-70 (reparto por equipo y lote de la Orden de Trabajo) **no
cambia**. HU-96 (ADR 0021) es la base: la exclusividad de lotes entre contratos
es lo que permite que la orden deje de validar choques, y el cierre de la última
aplicación completa la liberación que allí quedó manual.
...
contrato y quién los trabaja se vean sin salir a otras pantallas. Cambia una regla
de negocio: cerrar una orden exige trabajo registrado y terminado (adenda del ADR
0022). La numeración sigue desde HU-100 (HU-99 es la de lotes en bloque). Implementada
en la rama `feature/orden-ficha`.*

| ID | Historia / tarea | CA esenciales | Est. |
|---|---|---|---|
| HU-100 | Como **encargado**, quiero ver y cambiar el estado de la orden desde su ficha y su detalle con pasos que me guíen, y ver sus lotes, su contrato y qué equipo con qué dron los trabaja, para operar la aplicación sin ir de pantalla en pantalla | **1) Pasos:** en la ficha de EDICIÓN y en el DETALLE, `molecules/step-arrow` con «Emitida → Vigente → Pausada → Consumida → Cancelada» (`PasosDeOrden`; «Vencida» solo si ya lo está) y un párrafo de ayuda por estado (`operaciones.ordenes.estado_ayuda.*`); cada paso abre el mismo modal que el listado (`_orden-modales`, con el motivo de pausa y la causa y el motivo de cancelación); un permiso por acción (activar, pausar —también reanudar—, cerrar, cancelar) mediante las opciones opcionales `puedeIrA` y `modales` de `PasosDeEstado::armar()`; tono único `PasosDeOrden::TONO_POR_ESTADO`; activar vuelve al detalle. **2) Cabecera del detalle:** solo «Volver» y «Editar» (warning, mientras `emitida`); «Eliminar» es del listado. **3) Cierre:** `vigente → consumida` exige al menos un trabajo, todas las hectáreas asignadas a algún equipo y ningún trabajo abierto (`PoliticaCierreOrden`, guarda de `MaquinaEstadosOrden::cerrar()` dentro de la transacción); sin cumplirla, el paso y el «Cerrar» del listado abren un aviso que dice qué falta y a dónde ir, y el servidor lo sigue rechazando ante un pedido directo (adenda del ADR 0022). **4) Lotes:** en el formulario y en el detalle, de 20 en 20 (`paginarFilas`, como la tabla de lotes del contrato) y en orden natural —propiedad y luego código: L1, L2 … L10—; el detalle suma, después de las hectáreas, las columnas Equipo y Dron (un renglón por equipo; guion si el lote no tiene orden de trabajo; el dron es el que el equipo tenía asignado el día que empezó el trabajo, por `LecturaEquipoTrabajo::porIds()`/`recursosAFecha()`). **5) Contactos:** «Emitida por» solo lista los del cliente del contrato —nunca los de otros, ni ocultos— y el servidor rechaza un contacto de otro cliente (`ValidaContactoDelCliente`). **6) Edición:** la tarjeta del contrato (cliente, propiedades, aplicaciones pactadas, hectáreas y fechas) se ve también al editar. **7) Relacionado:** el detalle abre con el vínculo al contrato (tono warning); el aside de la edición lleva vínculos a crear o ver las órdenes de trabajo y a asignar o ver los equipos, según el estado y los permisos (se retira el botón «Asignar equipos» junto a la cantidad de equipos). **8) Listado:** filtros «Contrato» (cliente y número) y «Aplicación» (en palabras, hasta la mayor cantidad de aplicaciones de un contrato con órdenes) y «Volver a Contrato #N» cuando se llega desde el contrato (memento). **9) Resumen del contrato:** «Órdenes de aplicación» en plural, con «Ver más» que lleva el memento; la tarjeta de Orden de trabajo conserva sus datos sin «Ver más»; «Nueva orden» solo se activa sin aplicación abierta, es decir con la orden consumida o cancelada. **10) Corrección de una orden publicada:** una orden abierta (`emitida`, `vigente` o `pausada`) se puede editar —«Editar» en el detalle, el listado y la ficha—; en vigente o pausada la ficha avisa, pide un **motivo obligatorio** (`motivo_correccion`, `corregida_at`) y todo queda en la bitácora con el antes y el después; con trabajos, el tipo de insumo, la categoría y la dosis se ven pero no se cambian (`PoliticaEdicionOrden`, aplicada por `ActualizarOrden`, no solo por la pantalla); una cerrada redirige al detalle. **11) Causas de cancelación:** el modal registra Cliente, Dueño o Factor externo (antes «fuerza mayor»); cliente y dueño consumen el número de aplicación, el factor externo no (migración con el `CHECK` y el índice de numeración). **12) Menú lateral:** «Contratos» (Comercial) lleva un contador con los contratos «En ejecución» y «Órdenes de aplicación» (Operaciones) cuenta solo las órdenes vigentes; Comercial expone el dato por `Comercial\Contratos\LecturaContadoresPanel`, como los demás módulos. **13) Fronteras y tests:** Operaciones lee equipos solo por `Personal\Contratos\LecturaEquipoTrabajo`; `PasosDeOrdenTest`, `PoliticaCierreOrdenTest`, `PasosDeEstadoTest` | 3,5 d |

**Total: 3,5 d · 0 pantallas nuevas de menú (amplía Órdenes de aplicación y la ficha de Contratos)**
