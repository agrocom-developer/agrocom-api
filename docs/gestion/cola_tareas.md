...
**Última actualización: 2/9/2026 (tarea 30/31, planificación tras la 30).**
...
### Sprint 7 — de "destrabado" a "en marcha"
...
que califica para el ciclo automático — sus siete historias (HU-22 a HU-27,
...
Con el arquetipo formulario en `develop` (tarea 31) y el bug de LSP de
`atoms/input` en camino de resolverse (tarea 32, primera de la fila), Sprint 7
es mecánico: HU-22 (clientes, tarea 33), HU-23 (contratos, tarea 34), HU-24
(campos y lotes), HU-25 (órdenes), HU-26 (personas y bases), HU-27 (drones) y
HU-45 (usuarios — cierra la parte de HU-01 que quedó sin hacer). Cada una es
una HU entera con su PR, compuesta sobre las siete piezas del catálogo, y cada
una suma su spec visual propio en `tests/Visual/` (mismo patrón que
`organizacion.spec.ts`) para que la pantalla número 7 se vea como la número 1
sin revisión a ojo.

Solo se escribieron los prompts de 32, 33 y 34 en esta vuelta (regla de "las
próximas 3"). **HU-24, HU-25, HU-26, HU-27 y HU-45 siguen sin fila ni prompt**
— la próxima planificación, al cerrar la 34, les escribe el criterio de
aceptación ejecutable de cada una a partir de `plan_sprints.md` §165-181 y la
especificación funcional, siguiendo el mismo patrón que estableció la 33
...
**32 → 33 → 34, en ese orden, porque cada una es requisito de la siguiente.**
`atoms/input` (32) tiene que fusionar `$attributes` antes de que las siete
pantallas de Sprint 7 empiecen a repetir el envoltorio manual que documentó la
tarea 31 — es chica (2 etapas) y no depende de nada, así que va primera.
Clientes (33) va antes que contratos (34) porque `com_contratos.cliente_id` es
una FK real a `com_clientes`: aunque los tests de 34 puedan crear su propio
cliente por factory sin que exista la pantalla de alta, tiene sentido que el
patrón de ABM completo (el primero real del panel) se establezca una vez en la
HU más simple de las dos, y que contratos —con máquina de estados, 13 `CHECK`
y montos derivados— lo reutilice en vez de inventarlo en paralelo. HU-24 a
HU-27 y HU-45 quedan para la próxima vuelta de planificación, sin fila propia
todavía (ver "Sprint 7 — de destrabado a en marcha" arriba).
