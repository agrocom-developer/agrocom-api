...
pero su push se cortó y nunca tuvo PR. La 123 la integra en una rama nueva
(`feature/usuarios-dispositivos-panel`) y no en la vieja: el ciclo retoma una rama
con `git checkout`, y ese comando se niega si `cola_tareas.md` está modificado y
difiere entre `develop` y la rama (lo cambió el PR #264). Detrás de la 123 no queda
ninguna HU ni TE de `plan_sprints.md` que califique — ver «Fuera del ciclo
automático» y el análisis de la 122 —, así que cuando cierre corresponde
`runs/DETENER`. Las filas 95 y 102 a 109 estaban desactualizadas (decían «escrita»
o «pendiente» estando integradas o superadas): se corrigieron con el PR o el ADR
que lo prueba.

**Última actualización anterior: 14/9/2026 (planificación tras la tarea 104 — tareas
...
- **Resuelto (tarea 123): la tarea 121 no estaba integrada, aunque `runs/121.estado`
  dijera `OK`.** Usuarios, Dispositivos y Versiones de APK estaban hechos en la rama
  local `feature/panel-usuarios-dispositivos` (seis commits sobre `develop`,
  `bin/verify` en verde según su sesión), pero el push se cortó («unexpected
  disconnect»), el ciclo no pudo abrir el PR y siguió con la 122. La 123 la trajo con
  `git merge` a la rama `feature/usuarios-dispositivos-panel`: solo chocaron dos
...
| Orden de Trabajo: reparto «por experiencia» y cierre manual del Trabajo desde el panel | Condición 3. No hay ningún dato de experiencia en `per_personas` ni en la cuadrilla (falta definir qué la mide), y un cierre manual toca la máquina de estados del Trabajo, que hoy cierra la app de campo por sync. Lo decide el dueño |
| Arquetipo Detalle (plan de homogeneización §1.1) | Condición 3. Falta decidir si queda para objetos transaccionales especiales o se adopta en todo el sistema; de eso dependen los `show`, los `.ag-filtros` y los `confirm()` nativos que la homogeneización dejó fuera |
| Stock mínimo (hallazgo de la tarea 117) | Condición 3. «Bajo el mínimo» en Stock es siempre 0 porque `inv_stock.stock_minimo` solo se crea en 0: falta decidir dónde se fija y quién lo edita |
| Limitaciones de HU-97 (ADR 0022, «Consecuencias») | Condiciones 1 y 3. No es una HU/TE del plan y el arreglo pasa por el contrato con la app y una decisión de negocio: la app no se entera de una orden cancelada, pausada o cerrada; las sesiones abiertas no se frenan al pausar o cancelar; un lote agregado al contrato no entra en la aplicación abierta |
...
corresponde `runs/DETENER`. La 121 no llegó a integrarse (se cortó el push); su
cierre es la 123, y agotada esa es cuando corresponde `runs/DETENER`.
