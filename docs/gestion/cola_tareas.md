...
| 128 | `?q[]=x` (y `?estado[]=x`) deja de dar 500 en todos los listados: un helper de Plataforma, patrón de la 121 | `./bin/verify` = 0, `runs/128-sondas.log` sin 500 en todos los `index`, `grep "string('q')"` vacío en controladores | `Plataforma/**` (helper), controladores web de listado de todos los módulos (solo cómo leen el parámetro), tests | no | 2 | **completa, PR #281 abierto** (`feature/filtro-texto-listados`, `bin/verify` en verde, `mergeStateStatus=BLOCKED` por protección de rama — no es trabajo de esta sesión mergearlo) |
