...
**Agrocom SRL · Documento oficial vigente · Contexto de negocio de `docs/especificacion/especificacion_funcional_tecnica.md`**

*Migrado sin cambios de fondo desde `docs/legacy/ventana_al_negocio_fumigacion.md` (documento de trabajo original) — el contenido de negocio no depende de las decisiones técnicas que sí cambiaron entre documentos.*
...
1. **Instalación de base**: campamento en la propiedad, generador, Starlink, camioneta, drones, rotación de baterías, stock crítico de repuestos, bidones de gasolina. La "base" de la especificación es esto: un punto físico con conectividad desde donde se despliega al lote (donde NO hay señal).
...
Ver el banco completo de preguntas en `docs/gestion/banco_preguntas_por_rol.md`.

### 9.3 Qué validar puntualmente (checklist de la especificación y de este documento)
...
- Orden de mezcla real vs. la plantilla de 10 pasos de la especificación (agua → corrector pH → antiespumante → sólidos pre-disueltos → SC → SL → EC/EW/OD → aceites/coadyuvantes → antideriva → completar agua).
...
Tres conversaciones de ~1 hora (piloto + auxiliar juntos rinde bien: se corrigen entre ellos; el jefe de campo aparte), grabadas con permiso. De cada una salen tres listas: **confirmado** (la especificación ya lo modela bien), **corregido** (la especificación dice X, el campo hace Y — se ajusta), y **descubierto** (proceso que no estaba escrito — se decide si entra en v1 o se anota). Con eso, la captura de procesos que podría tomar semanas de idas y vueltas se cierra en una semana con evidencia.
