...
- La tarea 32 (el fix de LSP de `atoms/input`) **no llegó a cerrar** — se
  agotaron sus etapas sin integrarse (PR #76, borrador, `runs/32.estado` =
  `INCOMPLETA`). Antes de escribir ningún campo ancho, mirá si
  `resources/views/components/atoms/input.blade.php` ya fusiona `$attributes`
  en el `<div>` raíz (buscá `$attributes->except(['class'])` en el archivo).
  Si sí (alguien lo terminó a mano mientras tanto), pasá la clase directo al
  componente, sin envoltorio. Si no, replicá el patrón de la tarea 31: un
  `<div>` propio envolviendo ese campo puntual en la página — **no toques
  `atoms/input`**, arreglarlo no es mandato de esta tarea (blast radius: todo
  el panel lo consume).
