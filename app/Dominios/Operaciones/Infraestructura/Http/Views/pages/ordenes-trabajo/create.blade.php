{{--
    ...
    `ordenes-trabajo-form.js` recarga esta misma pantalla con `?orden_id=`. Sin
    orden elegida solo se ve el selector.
        ...
        >
            ...
            @if ($ordenElegida === null)
                <x-molecules.alert-strip variant="info" icon="info">
                    {{ __('operaciones.ordenes_trabajo.sin_orden_aviso') }}
                </x-molecules.alert-strip>
            @else
                <x-molecules.form-section
            ...
            {{-- Los botones van en el slot `actions`: el organismo no pinta el slot
                 ...
                 se dibujaba nunca). Sin orden elegida no hay nada que confirmar. --}}
            @if ($ordenElegida !== null)
