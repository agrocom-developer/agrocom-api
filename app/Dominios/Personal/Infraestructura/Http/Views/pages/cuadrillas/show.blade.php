{{--
    ...
    salta a "hoy" — ver `CuadrillasController::volverAFicha()`.
    ...
    resources/css/pages/cuadrillas.css — cero color hardcodeado
...
<x-templates.panel-shell :title="__('personal.equipos_trabajo.ficha_titulo', ['codigo' => $equipo->codigo])" :tema="$tema">
    ...
    >
        <div class="ag-cuadrillas-ficha">
            ...
            @if (session('aviso'))
                <x-molecules.alert-strip variant="warning" icon="warning" class="ag-cuadrillas-ficha__aviso">
                    {{ session('aviso') }}
                </x-molecules.alert-strip>
            @endif

            @if ($errors->has('persona_id') || $errors->has('recurso_id') || $errors->has('hasta'))
                <x-molecules.alert-strip variant="danger" icon="error" class="ag-cuadrillas-ficha__aviso">
                    {{ $errors->first('persona_id') ?: $errors->first('recurso_id') ?: $errors->first('hasta') }}
                </x-molecules.alert-strip>
            @endif

            <div class="ag-cuadrillas-ficha__resumen">
                <span class="ag-cuadrillas-ficha__campo">
            ...
            <form method="GET" action="{{ route('panel.cuadrillas.show', $equipo) }}" class="ag-cuadrillas-ficha__selector-fecha">
                <x-atoms.date
                    name="fecha"
            ...
            {{-- Integrantes --}}
            <div class="ag-cuadrillas-ficha__seccion">
                ...
                @else
                    <div class="ag-cuadrillas-ficha__lista">
                        @foreach ($integrantes as $integrante)
                            <div class="ag-cuadrillas-ficha__fila">
                                ...
                                @if ($puedeEditar)
                                    <form
                                        method="POST"
                                        action="{{ route('panel.cuadrillas.integrantes.destroy', [$equipo, $integrante->id]) }}"
                                        class="ag-cuadrillas-ficha__finalizar"
                                    >
                                        @csrf
                                        @method('DELETE')
                ...
                @if ($puedeEditar)
                    <form method="POST" action="{{ route('panel.cuadrillas.integrantes.store', $equipo) }}" class="ag-cuadrillas-ficha__alta">
                        @csrf
                        <input type="hidden" name="fecha" value="{{ $fecha }}">
            ...
            <div class="ag-cuadrillas-ficha__seccion">
                        ...
                        @foreach ($recursos as $recurso)
                            <div class="ag-cuadrillas-ficha__fila">
                                ...
                                @if ($puedeEditar)
                                    <form
                                        method="POST"
                                        action="{{ route('panel.cuadrillas.recursos.destroy', [$equipo, $recurso->id]) }}"
                                        class="ag-cuadrillas-ficha__finalizar"
                                    >
                                        @csrf
                                        @method('DELETE')
                ...
                @if ($puedeEditar)
                    @foreach ($tiposRecurso as $tipo)
                        <form method="POST" action="{{ route('panel.cuadrillas.recursos.store', $equipo) }}" class="ag-cuadrillas-ficha__alta">
                            @csrf
                            <input type="hidden" name="fecha" value="{{ $fecha }}">
